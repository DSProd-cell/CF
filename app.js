...
function passesSecondary(course) {
  ...
  const eduActive = state.eduPref.ug || state.eduPref.pg;
  if (eduActive) {
    const wantsUG = state.eduPref.ug && course.educationLevel === 'UG';
    const wantsPG = state.eduPref.pg && course.educationLevel === 'PG';
    if (!wantsUG && !wantsPG) return false;
  }
  return true;
}
...
function renderSidebar() {
  ...
  const educationBody = `
    <label class="flex items-center gap-2 text-sm cursor-pointer mb-2">
      <input type="checkbox" id="eduUG" class="pretty w-4 h-4 cursor-pointer" ${state.eduPref.ug ? 'checked' : ''} />
      UG
    </label>
    <label class="flex items-center gap-2 text-sm cursor-pointer">
      <input type="checkbox" id="eduPG" class="pretty w-4 h-4 cursor-pointer" ${state.eduPref.pg ? 'checked' : ''} />
      PG
    </label>`;
  ...
  wrap.innerHTML = [
    ...
    accordionSection('education', 'Education Preference', null, educationBody),
  ...
  const eduUGCb = document.getElementById('eduUG');
  if (eduUGCb) eduUGCb.onchange = (e) => { state.eduPref.ug = e.target.checked; renderAll(); };
  const eduPGCb = document.getElementById('eduPG');
  if (eduPGCb) eduPGCb.onchange = (e) => { state.eduPref.pg = e.target.checked; renderAll(); };
...
function cohortBadgeLabel(course) {
  if (state.source === 'career') {
    return careerMatchPercent(course) >= CAREER_HIGH_MATCH_THRESHOLD ? 'High Match' : 'Good Match';
  }
...
function renderShortlistCount() {
  ...
  const count = state.shortlist.size + cimActiveCount;
  document.getElementById('bottomBarCount').textContent = `${count} Course${count === 1 ? '' : 's'} added`;
}
